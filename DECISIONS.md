...
41. **Self-updates come from GitHub Releases** (`inc/updater.php`): latest release via the API, cached 6 h, injected into `update_themes`; `upgrader_source_selection` keeps the folder name `eurasiapulse`; auto-install is on by default; `Update URI` points at the repository so wordpress.org never offers a same-slug theme. Default repository: `kazprose/eurasiapulse`. Sites running 1.0.0 have no updater yet and must install the current release once by hand. **The repository turned out to be private**, so 1.1.1 adds token support (`EURASIAPULSE_GITHUB_TOKEN`: authenticated API calls, download through the API asset URL, the storage redirect fetched without the token via `upgrader_pre_download`). Both paths were verified against a local mock of the GitHub API serving the real release asset; after the repository was made public, the live path (real API, real asset, 1.0.9 → 1.1.1 on WordPress 6.9.8) was verified too. No token is needed for the public repository.
