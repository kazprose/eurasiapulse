...
2. The repository is public now, so no token is needed. (For a private repository, `define( 'EURASIAPULSE_GITHUB_TOKEN', '…' );` in `wp-config.php` with a fine-grained token limited to the repository, *Contents: read*, would be required.)
